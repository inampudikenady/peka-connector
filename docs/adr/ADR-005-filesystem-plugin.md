...
Implement a filesystem plugin supporting PDF, DOCX, TXT, and Markdown extensions. Require absolute readable directories, apply include/exclude globs, do not follow symlinks, and collect relative path, filename, extension, size, modification time, and SHA-256. Persist a complete metadata snapshot per source. Mount customer directories read-only in Docker.
