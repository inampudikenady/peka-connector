...
import {
  Alert, Box, Button, CircularProgress, Chip, IconButton, Paper, Stack, Table,
...
export function KnowledgeStoreCard({ store, refreshing = false, onRefresh }: {
  store: KnowledgeStoreOverview;
  refreshing?: boolean;
  onRefresh?: () => void;
}) {
  ...
  return <Paper variant="outlined" sx={{ flex: '1.8 1 340px', minWidth: 0, px: 2, py: 1.5 }}>
    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1.25}>
      ...
      <Stack direction="row" alignItems="center" spacing={0.5}>
        {onRefresh && <Tooltip title="Refresh knowledge store"><span><IconButton size="small" aria-label="Refresh knowledge store" disabled={refreshing} onClick={onRefresh}><RefreshIcon fontSize="small" sx={refreshing ? { animation: 'knowledge-store-refresh 0.8s linear infinite', '@keyframes knowledge-store-refresh': { to: { transform: 'rotate(360deg)' } } } : undefined} /></IconButton></span></Tooltip>}
...
export function OverviewPage() {
  ...
  const { data, error, loading, refreshing, retrying, refresh, retryHeartbeat } = useConnectorStatus();
  ...
  return <Stack spacing={2.5}>
    ...
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5 }}><KnowledgeStoreCard store={data.knowledge_store} refreshing={refreshing} onRefresh={user?.role === 'administrator' ? () => void refresh() : undefined} /><SummaryValue label="Enabled integrations" value={data.enabled_integration_count} /><SummaryValue label="Healthy integrations" value={data.healthy_integration_count} /><SummaryValue label="Need attention" value={data.attention_integration_count} /></Box>
