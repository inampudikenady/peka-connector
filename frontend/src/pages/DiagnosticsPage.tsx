...
export function DiagnosticsPage() {
  ...
  const facts = [
    ...
    ['Latest Heartbeat Failure', data.latest_heartbeat_failure_reason ?? 'None'],
  ...
  return <Stack spacing={3}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between"><div><Typography variant="h4" fontWeight={700}>Diagnostics</Typography><Typography color="text.secondary">Runtime and appliance health checks</Typography></div><Button startIcon={<DownloadIcon />} variant="outlined" onClick={() => void api.downloadDiagnostics().catch((e: Error) => toast.show(e.message, 'error'))}>Download diagnostics bundle</Button></Stack>
    ...
    <Paper variant="outlined"><Stack sx={{ p: 2, pb: 0 }}><Typography variant="h6">Source and Local Runtime Health</Typography><Typography variant="body2" color="text.secondary">These checks are independent of connector-to-PEKA connectivity.</Typography></Stack><List>{data.checks.map((check) => <ListItem key={check.name} divider><ListItemText primary={check.name} secondary={check.detail} /><Chip label={check.status} color={check.status === 'healthy' ? 'success' : check.status === 'unhealthy' ? 'error' : 'default'} /></ListItem>)}</List></Paper>
